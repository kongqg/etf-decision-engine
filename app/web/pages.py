...
from app.web.presenters import page_context, serialize_advice_record, serialize_explanations
...
def home(request: Request, status: str | None = Query(default=None), db: Session = Depends(get_db)):
    ...
    advice = serialize_advice_record(get_latest_advice(db))
...
def evidence_page(request: Request, status: str | None = Query(default=None), db: Session = Depends(get_db)):
    ...
    try:
        ...
        context["advice"] = serialize_advice_record(get_advice_by_id(db, evidence["advice_id"]))
    except ValueError:
...
def portfolio_page(request: Request, status: str | None = Query(default=None), db: Session = Depends(get_db)):
    ...
    context["advice"] = serialize_advice_record(get_latest_advice(db))
...
def decide_now_action(db: Session = Depends(get_db)):
    try:
        ...
        return RedirectResponse(url=f"/advice/{advice['id']}?status=建议已生成", status_code=303)
